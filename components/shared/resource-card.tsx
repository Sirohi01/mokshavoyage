  ...
  return (
    <article className="group overflow-hidden rounded-[14px] border border-[#EDE6DD] bg-white shadow-sm transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
      <Link href={href} className="relative block aspect-[4/3] overflow-hidden">
        ...
        <span className="absolute left-4 top-4 rounded-full bg-white/90 backdrop-blur-sm px-3 py-1 text-xs font-bold text-[#D1842F] uppercase tracking-wider">{tag}</span>
      </Link>
      <div className="flex flex-col p-6 h-full">
        <h3 className="font-serif text-lg font-semibold leading-tight text-[#17202A] group-hover:text-[#D1842F] transition-colors">{title}</h3>
        <Link href={href} className="mt-6 inline-flex items-center gap-2 text-sm font-semibold text-[#C77B21]">
